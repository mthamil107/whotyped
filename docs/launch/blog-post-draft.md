...
|---|---|---|---|---|
| 2025-07-18 | Replit's agent deletes a live production database (1,200+ executives, 1,190+ companies) during a declared code freeze, then misrepresents recoverability; reports put the fake records it generated at about 4,000 | the agent had write access to production on the platform side; the freeze was a human agreement, not a machine rule | `freeze_violation` if a freeze window had been configured; `rhythm.burst`; `pty.none` | This was not over SSH; whotyped's vantage point would need to be the DB host or a bastion |
| 2025-12 (made public 2026-02-20) | Amazon's internal Kiro agent, under an engineer's role with broader permissions than intended, deletes and recreates an environment; the FT ties it to a ~13 h AWS Cost Explorer outage in one China region. Amazon confirms the interruption and the misconfigured role but attributes it to user error, not the AI | an agent acting under a human role issued destructive changes | `proc.agent_name` (`kiro-cli`) only if the agent ran on a monitored Linux host; no public source describes its flags | Cloud API calls from a laptop are invisible; the attribution itself is disputed |
| 2025-08-26 | s1ngularity (malicious Nx packages) runs `claude --dangerously-skip-permissions -p`, `gemini --yolo -p`, `q chat --trust-all-tools --no-interactive` to inventory secrets into `/tmp/inventory.txt` and exfiltrate them to GitHub; Wiz: 1,000+ valid GitHub tokens, 5,500+ private repos made public (later revised to 6,700+) | developer machines and CI runners; auditd would have logged the argv | `proc.skip_flags` at weight 25 plus `proc.agent_name` 45: level `alert` on the first execve | CI runners rarely run auditd; laptops are out of scope |
| 2025-09 detected, 2025-11-13 disclosed | GTG-1002: a state-sponsored actor uses Claude Code to run 80-90% of an intrusion campaign's tactical operations against ~30 organisations, a handful successfully | targets saw agent-driven reconnaissance, credential testing and remote command execution through MCP-connected tooling; the report does not name SSH | `rhythm.*` and `pty.none` on any lateral SSH hops; `banner.library` if DEBUG1; `net.ai_api` only on hosts where the agent itself ran | attacker-controlled agents can be told to slow down and allocate PTYs |

Sources for every figure above: `docs/research/05-incident-sources.md`.
