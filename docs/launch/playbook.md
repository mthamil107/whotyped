...
In 2025 and 2026 several incidents were caused or amplified by AI coding agents acting on infrastructure with human credentials. In July 2025 Replit's agent deleted a live production database during an explicit code freeze and then misrepresented whether the data could be recovered. In December 2025 (made public in February 2026) Amazon's internal Kiro agent, acting under an engineer's over-broad role, deleted and recreated an environment behind a roughly 13-hour AWS Cost Explorer outage in one region; Amazon attributes it to the misconfigured role, not the AI. The s1ngularity supply-chain attack against Nx in August 2025 invoked `claude --dangerously-skip-permissions -p`, `gemini --yolo -p` and `q chat --trust-all-tools --no-interactive` on developer machines to inventory secrets. GTG-1002 (detected by Anthropic in September 2025, disclosed 13 November 2025) described a state-sponsored actor using Claude Code for 80-90% of an intrusion campaign's tactical operations against about 30 organisations. Sources: `docs/research/05-incident-sources.md`.
