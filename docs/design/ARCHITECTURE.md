...
Rules: `alert` needs at least two categories > 0. Negative clue `pty.interactive` is dropped when `process` > 0. Declared agents (`AI_AGENT` seen) are `class=declared_agent`, level info, regardless of score; inside a freeze window any declared or >=40 suspected activity is `freeze_violation` at level high. Re-alert: on first crossing of 40/70/90, then at most one `agent_still_active` per 30 min, `agent_ended` on expiry.
